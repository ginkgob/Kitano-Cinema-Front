  ...
  return (
    <section className='w-[80vw] h-fit bg-white rounded-2'>
      <div className='grid grid-cols-5 content-center justify-items-center m-4'>
        ...
        <div className='m-2 w-40 h-auto hover:scale-[1.7] transition duration-500 hover:z-10' key={movie.id}> 
          ...
          <p key={i} style={{visibility:isHover[`${i}`] ? 'hidden' : 'visible'}} className='text-center text-gray-600 p-2'>{movie.name}</p>
        </div>
        )
      }
      </div>
